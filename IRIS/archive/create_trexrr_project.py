...
helpers_content = "" # This will be read from helpers_content.txt
system_diagnostics_content = "" # This will be read from system_diagnostics_content.txt
main_app_content = "" # This will be read from main_app_content.txt
...
try:
    ...
    if app_path.exists():
        ...
        logger.error(f"❌ Failed to create directory '{app_path}'. Check permissions.")
        # If the app_path cannot be created, stop here as subsequent steps will fail
        sys.exit(1) 
    ...
    if utils_path.exists():
        ...
        logger.error(f"❌ Failed to create directory '{utils_path}'. Check permissions.")
        sys.exit(1)
    ...
    if modules_path.exists():
        ...
        logger.error(f"❌ Failed to create directory '{modules_path}'. Check permissions.")
        sys.exit(1)
    ...
    files_to_read_and_create = {
        ...
        script_dir / "system_diagnostics_content.txt": modules_path / "system_diagnostics.py",
    ...
    for source_file, destination_file in files_to_read_and_create.items():
        try:
            logger.info(f"  Reading content from '{source_file}' and writing to '{destination_file}'")
            # Ensure proper encoding when reading/writing text files
            content = source_file.read_text(encoding='utf-8')
            with open(destination_file, "w", encoding='utf-8') as f:
                f.write(content)
            logger.info(f"  ✅ Successfully wrote '{destination_file}'.")
        except FileNotFoundError:
            logger.error(f"  ❌ Error: Source content file '{source_file}' not found. Please ensure it exists in the same directory as this script.")
            sys.exit(1) # Critical error, cannot proceed if content files are missing
    ...
    for init_file_path in empty_init_files:
        try:
            logger.info(f"  Creating empty __init__.py file: '{init_file_path}'")
    ...
    logger.info(f"You can now run the main application from: '{app_path / 'main_app.py'}'")

except Exception as e:
    logger.critical(f"A critical error occurred during project setup: {e}")

# This part only runs if the script is executed directly
# It attempts to launch the created main_app.py if it exists and setup was successful
if app_path.exists() and (app_path / "main_app.py").exists():
    logger.info("Attempting to launch the IRIS Rapid Response GUI now...")
    try:
        # Correctly launch as a module from the project root using `python -m` and `cwd`
        subprocess.Popen([sys.executable, "-m", "IRISX_app.main_app"], cwd=PROJECT_ROOT)
        logger.info(f"GUI launch command sent with cwd='{PROJECT_ROOT}'. Check for a new window.")
    except Exception as e:
        logger.error(f"Failed to launch main_app.py: {e}")
        logger.info(f"Please navigate to '{PROJECT_ROOT}' and run 'python3 -m IRISX_app.main_app' manually.")
else:
    logger.warning("main_app.py was not found after creation. GUI will not be launched automatically.")